...
export default function Contacts(props) {
    ...
    useEffect( ( ) => {
        ...
        props.setContacts( localStorage.getItem('contacts'))

        fetch("http://localhost:5000/interface/getContacts", {
            method: "GET",
                ...
                <div className="h-full w-full">
                    <div className="relative w-full mx-auto bg-white shadow-lg rounded-lg">
                        <header className="pt-6 pb-4 px-5 border-b border-gray-200">
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <div className='flex items-center gap-4 '>
                                    <button onClick={ e => { props.setContacts(false); localStorage.removeItem('contacts'); }} className='bg-transparent hover:bg-primary border border-primary hover:text-white transition-all duration-300 px-6 py-2 text-black rounded-3xl' >Chats</button>
                                    <button className='bg-primary px-6 py-2 text-white rounded-3xl' >Contacts</button>
                        ...
                        <div className="py-3 px-5">
                            <h3 className="text-xs font-semibold uppercase text-gray-400 mb-1">Contacts</h3>
                            <div className="divide-y divide-gray-200">
                                {
                                    chatHeads.length && chatHeads.map( ( chat, index ) => {
                                        if ( props.loggedIn === chat._id ) return null
                                        return (
                                            <ChatHead setContacts={ props.setContacts } key={ index } loggedIn={ props.loggedIn } setChattingDisplayName={ props.setChattingDisplayName } ThreadID={ chat._id } setChatting={ props.setChatting } ThreadName={ chat.username } Message={ "Come Say Hi" } />
                                        )
                                    })
                                }
                            </div>
